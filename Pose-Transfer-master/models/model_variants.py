...
# This are the non-matching weights!!
...
class PATNModel(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, use_gpus=False, padding_type='reflect', n_downsampling=2):
...
class PATNetwork(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, use_gpus=False, padding_type='reflect', n_downsampling=2):
        ...
        self.use_gpus = use_gpus
        self.model = PATNModel(input_nc, output_nc, ngf, norm_layer, use_dropout, n_blocks, self.use_gpus, padding_type, n_downsampling=n_downsampling)

    def forward(self, input):
        if self.use_gpus and isinstance(input[0].data, torch.cuda.FloatTensor):
