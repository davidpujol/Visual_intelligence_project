...
class SSDScorer(object):
    def __init__(self, model_def='deploy.prototxt', model_weights='VGG_VOC0712_SSD_300x300_iter_120000.caffemodel'):
        ...
        # input preprocessing: 'data_processing' is the name of the input blob == net.inputs[0]
        self.transformer = caffe.io.Transformer({'data_processing': self.net.blobs['data_processing'].data.shape})
        self.transformer.set_transpose('data_processing', (2, 0, 1))
        self.transformer.set_mean('data_processing', np.array([104,117,123])) # mean pixel
        self.transformer.set_raw_scale('data_processing', 255)  # the reference model operates on images in [0,255]
        self.transformer.set_channel_swap('data_processing', (2,1,0))  # the reference model has channels in BGR order

        # set net to batch size of 1
        self.img_side = 300
        self.net.blobs['data_processing'].reshape(1,3,self.img_side,self.img_side)

    def get_score(self, image, image_class):
        image = img_as_float(image)
        transformed_image = self.transformer.preprocess('data_processing', image)
        self.net.blobs['data_processing'].data[...] = transformed_image
